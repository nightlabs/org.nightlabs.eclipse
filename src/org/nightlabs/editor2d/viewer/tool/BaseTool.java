...
import org.nightlabs.editor2d.viewer.util.ToolUtil;
	...
	protected JToolTip createToolTip(DrawComponent dc, String text, int x, int y) 
	{
		...
		if (text == null) 
		{
			if (dc != null) {
				...
				toolTip.setTipText(
						Messages.getString("org.nightlabs.editor2d.viewer.tool.BaseTool.tooltip.name")+": "+dc.getName() + "\n" + //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
						Messages.getString("org.nightlabs.editor2d.viewer.tool.BaseTool.tooltip.id")+": "+dc.getId());				 //$NON-NLS-1$ //$NON-NLS-2$
			}			
		}
		else {
