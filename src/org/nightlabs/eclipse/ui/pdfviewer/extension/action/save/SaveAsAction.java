...
//	}
	...
	public void calculateEnabled() {
		final PDFViewer pdfViewer = getPDFViewerActionRegistry().getPDFViewer();
	...
	public void runWithEvent(final Event event) {
		final PDFViewer pdfViewer = getPDFViewerActionRegistry().getPDFViewer();
		final Collection<SaveAsActionHandler> contextElements = pdfViewer.getContextElements(SaveAsActionHandler.CONTEXT_ELEMENT_TYPE);
		if (contextElements.isEmpty()) {
			setEnabled(false);
			return;
		}
		final SaveAsActionHandler saveAsActionHandler = contextElements.iterator().next();
