	...
	protected void doSetValue(Object value) {
		...
		try {
			if (value == null) {
				...
				throw new IllegalArgumentException("value must be an instance of java.util.Date or null! " + value);
		...
		text.addFocusListener(new FocusAdapter() {
			...
			public void focusLost(FocusEvent event) {
				if (textModified) {
					...
					Date date;
					try {
						date = DateFormatter.parseDate(txt);
					} catch (DateParseException e) {
						MessageDialog.openError(text.getShell(), "Invalid date", String.format("The text \"%s\" cannot be interpeted as a date!", txt));
						return;
					}
					setValue(date);
				}
