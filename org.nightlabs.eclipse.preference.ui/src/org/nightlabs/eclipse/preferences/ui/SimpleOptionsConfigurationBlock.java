 ...
 * </p>
 * @author unascribed
 * @version $Revision: 1955 $ - $Date: 2008-01-24 17:28:33 +0100 (Do, 24 Jan 2008) $
 * @since 2.1
 */
public abstract class SimpleOptionsConfigurationBlock {
	...
	public SimpleOptionsConfigurationBlock(IStatusChangeListener context, IProject project, Key[] allKeys, IWorkbenchPreferenceContainer container) {
		fContext= context;
		fProject= project;
		fAllKeys= allKeys;
	...
	private void testIfOptionsComplete(Key[] allKeys) {
		for (int i= 0; i < allKeys.length; i++) {
			if (allKeys[i].getStoredValue(fLookupOrder, false, fManager) == null) {
				PreferencesUIPlugin.logErrorMessage("preference option missing: " + allKeys[i] + " (" + this.getClass().getName() +')');  //$NON-NLS-1$//$NON-NLS-2$
			}
		}
	}
	
	private int getRebuildCount() {
		return fManager.getWorkingCopy(new DefaultScope().getNode(PreferencesUIPlugin.PLUGIN_ID)).getInt(REBUILD_COUNT_KEY, 0);
	}
	
//	private void incrementRebuildCount() {
//		fRebuildCount++;
//		fManager.getWorkingCopy(new DefaultScope().getNode(PreferencesUIPlugin.PLUGIN_ID)).putInt(REBUILD_COUNT_KEY, fRebuildCount);
//	}
	...
	protected Combo addComboBox(Composite parent, String label, Key key, String[] values, String[] valueLabels, int indent) {
		GridData gd= new GridData(GridData.HORIZONTAL_ALIGN_BEGINNING, GridData.VERTICAL_ALIGN_BEGINNING, false, false, 1, 1);
		gd.horizontalIndent= indent;
...
//			}
	...
	protected void resetToDefaults(Set<Key> keysToReset)
	{
		...
		for (Key key : keysToReset) {
			String oldValue = getValue(key);
			String defValue = key.getStoredValue(fLookupOrder, true, fManager);
			setValue(key, defValue);
			validateSettings(key, oldValue, defValue);
		}
	...
	public void performDefaults() {
		for (int i= 0; i < fAllKeys.length; i++) {
			...
			validateSettings(curr, oldValue, defValue);
		}
	...
	public void performRevert() {
		for (int i= 0; i < fAllKeys.length; i++) {
			...
			validateSettings(curr, oldValue, origValue);
		}
	...
	protected boolean isProjectBlock()
	{
